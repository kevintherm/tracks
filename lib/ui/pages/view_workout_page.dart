...
class ViewWorkoutPage extends StatefulWidget {
  ...
  const ViewWorkoutPage({super.key, required this.workout});
...
class _ViewWorkoutPageState extends State<ViewWorkoutPage> {
  ...
  void onStartSession() async {
    final confirm = await showModalBottomSheet(
      context: context,
      builder: (_) => _ConfirmStartSessionDialog(workout: workout),
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: SafeArea(
        child: FutureBuilder<List<_ExerciseParam>>(
          future: getExercises(),
                  ...
                  flexibleSpace: LayoutBuilder(
                    builder: (context, constraints) {
                      ...
                      final scrollProgress =
                          ((maxHeight - minHeight) / (280 - minHeight)).clamp(
                            0.0,
                            1.0,
                          );
                            ...
                            Container(
                              decoration: BoxDecoration(
                                gradient: LinearGradient(
                                  ...
                                  stops: const [0.0, 0.25, 0.5, 0.95, 1.0],
                                  colors: [
      ...
      floatingActionButton: _FabMenu(onStartSession: onStartSession),
    );
  }
}
...
class _FabMenu extends StatelessWidget {
  ...
  const _FabMenu({required this.onStartSession});
  ...
  Widget build(BuildContext context) {
    return Pressable(
      ...
      child: Container(
        ...
        child: Icon(MingCute.play_line, color: Colors.white),
      ),
    );
  }
}
...
class _ConfirmStartSessionDialog extends StatelessWidget {
  const _ConfirmStartSessionDialog({required this.workout});
